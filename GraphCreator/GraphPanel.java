...
public class GraphPanel extends JPanel {
	...
	public ArrayList<String> getConnectedLabels(String label) { //gets the label
	...
	public void printAdjacency() {
		...
		for (int a = 0; a < adjacency.size(); a++) {
			for (int b = 0; b < adjacency.size(); b++) {
				System.out.print(adjacency.get(a).get(b) + "\t"); //prints out the adjacency matrix
	...
	public Node getNode(String s) { //gets the label of the node
		for (int a = 0; a < nodeList.size(); a++) {
			Node node = nodeList.get(a);
			if (s.equals(node.getLabel())) {
				return node;
	...
	public boolean nodeExists(String s) {          //checks if node really exists
		for (int a = 0; a < nodeList.size(); a++) {
	...
	public void paintComponent(Graphics g) {
		...
		for (int a = 0; a < edgeList.size(); a++) {
			g.setColor(Color.BLACK);
			g.drawLine(edgeList.get(a).getFirst().getX(), edgeList.get(a).getFirst().getY(), //draws the connected line
					edgeList.get(a).getSecond().getX(), edgeList.get(a).getSecond().getY());

			int fx = edgeList.get(a).getFirst().getX();            //gets xy for line
